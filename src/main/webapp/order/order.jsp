			...
			<table class="table table-striped">
				<thead>
					<tr>
						...
						<th class="${order.status == 1 ? "" : "operation"}n">操作</th>
					</tr>
				</thead>
				<tbody>
					<c:forEach var="order" items="${requestScope.page.records}">
						<tr>
							...
							<td class="${order.status == 1 ? "" : "operation"}">
								...
								<c:if test="${order.status == 1}">
									<button type="button" class="btn btn-success btn-icon btn-sm"  data-toggle="modal" data-target="#receiveOrderModal" data-order-id="${order.orderId}">
		...
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				<div class="modal-header border-bottom-0">
					<h5 class="modal-title" id="receiveOrderModalLabel">提示</h5>
				...
				<div class="modal-footer border-top-0">
					...
					<a id="receiveOrderBtn" class="btn btn-primary btn-sm" href="javascript:void(0);">确定</a>
				</div>
			</div>
		</div>
	</div>
